...
* `calculate_region_statistics(df, case_times)` — агрегація на рівні Region
  ...
  відкидаються. Якщо один Case ID містить кілька різних значень Region
  (порушення припущення "1 кейс = 1 регіон"), це трактується як проблема
  якості даних: кейс цілком переноситься в `"Unknown"` (а не довільно бере
  перше значення), і кількість таких кейсів повертається окремим полем
  `inconsistent_region_cases_count` для прозорості.
...
* `region_analysis()` — Lead Time / Rework / Bottleneck / Waiting Time /
  Workload по регіонах; лідер і аутсайдер визначаються композитним
  рейтингом (Lead Time ×2 + Rework Rate + Waiting Time + Bottleneck share —
  жоден регіон не може стати "лідером" лише за рахунок низького Lead Time,
  ігноруючи інші метрики); окремо визначається `best_practice_region`
  (регіон, який одночасно нижче середнього за всіма чотирма метриками);
  порівняння з середнім/медіанним по всьому датасету; автоматичні insights
  та рекомендації; єдиний `kpi_summary` (кількість регіонів, найкращий/
  найгірший регіон, середній Lead Time по процесу, регіон з найвищим
  Rework Rate, регіон з найвищою концентрацією bottleneck) — і UI, і PDF
  читають ці шість чисел з одного місця, а не рахують їх повторно
  (лише якщо є колонка **Region**);
